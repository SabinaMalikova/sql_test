...
alter table owners add constraint unique_email unique (email);
...
select * from owners o inner join houses h on o.id = h.owner_id order by length(o.first_name) desc limit 1;
...
select *, h.* from addresses a inner join houses h on a.id = h.address_id where h.address_id in (5, 6, 7, 8, 9, 10);
-- - 4. Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.
select h as house, concat(o.last_name,' ',o.first_name) as owner_full_name, concat(c.last_name,' ',c.first_name) as client_full_name, a.name as agent_name from rent_info r
...
select * from customers c where c.date_of_birth > '1999-01-01' limit 15 offset 10;
-- - 6. Рейтинги боюнча уйлорду сорттоп, уйлордун тайптарын, рейтингин жана уйдун ээлерин чыгар. (asc and desc)
select h.house_type, h.rating, concat(o.first_name, ' ', o.last_name) as owner_name from houses h inner join owners o on h.owner_id = o.id order by rating desc;
-- - 7. Уйлордун арасынан квартиралардын (apartment) санын жана алардын баасынын суммасын чыгар.
select h.house_type, count(house_type), sum(price) from houses h where house_type = 'Apartment' group by  h.house_type;
...
select ag, a, h.address_id from houses h inner join addresses a on h.address_id = a.id inner join agencies ag on a.id = ag.address_id where ag.name ilike 'My House';
-- - 9. Уйлордун арасынан мебели бар уйлорду, уйдун ээсин жана уйдун адрессин чыгар.
select h.*, o.first_name, ' ', o.last_name from houses h join owners o on h.owner_id = o.id inner join addresses a on h.address_id = a.id
...
select h.id as house_id, h.house_type, h.description, a.name as agency_name from houses h left join rent_info r on h.id = r.house_id inner join agencies a on r.agency_id = a.id
                                                                                          left join customers c on c.id = r.customer_id
where customer_id is null;
-- - 11.Клиенттердин улутуна карап, улутуну жана ал улуутта канча клиент жашайт санын чыгар.
select r.agency_id AS agency_id, count(r.customer_id) AS client_count from rent_info r group by r.agency_id;
-- - 12.Уйлордун арасынан рейтингтери чон, кичине, орточо болгон 3 уйду чыгар.
select id, house_type, description, rating from houses order by rating desc limit 3;
...
select round(avg(price)) as avg_home_price from houses;
...
select h.*, a.*,c.* from houses h inner join addresses a on h.address_id = a.id inner join customers c on a.city = 'Bishkek' and h.id = c.id;
-- - 20.Жендерине карап группировка кылыныз.
select gender, concat(c.first_name,' ',c.last_name) as customers_full_name from customers c group by gender, concat(c.first_name,' ',c.last_name) ;
